...
class TetrisGame : public IGame {
  ...
  private:
    ...
    uint8_t  _pieceType, _pieceRot, _pieceColor;
    uint8_t  _nextType;

    // 7-Bag — Seed als Instanzvariable (kein static → kein Reset-Bug)
    uint8_t  _bag[7];
    uint8_t  _bagPos;
    uint16_t _bagSeed;

    bool     _gameOver;
    uint8_t  _gameOverAnim;   // 0..TET_ROWS: rote Welle von unten
    uint8_t  _fallTimer, _fallCount;
    uint8_t  _level;
