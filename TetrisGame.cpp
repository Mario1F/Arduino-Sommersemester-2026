...
static uint16_t _tetSeed = 42;
static uint16_t _tetRng() { _tetSeed = _tetSeed * 6364 + 1; return _tetSeed; }
...
void TetrisGame::reset() {
  ...
  _linesCleared = 0;
  _inputTimer   = 0;
  _bagPos       = 7;  // erzwingt shuffle beim ersten Aufruf
...
void TetrisGame::_spawnPiece() {
  ...
  // Game Over nur wenn der Block an seiner Startposition SOFORT blockiert
  // → das Feld ist buchstaeblich randvoll
...
void TetrisGame::handleInput(int8_t dx, int8_t dy, bool btn) {
  ...
  if (_inputTimer > 0) { _inputTimer--; }
    ...
    if (dx == -1 && _canPlace(_pieceR, _pieceC - 1, _pieceType, _pieceRot)) {
      _pieceC--; _inputTimer = 4;
    }
    if (dx ==  1 && _canPlace(_pieceR, _pieceC + 1, _pieceType, _pieceRot)) {
      _pieceC++; _inputTimer = 4;
    }
    if (dy ==  1) {
      if (_canPlace(_pieceR + 1, _pieceC, _pieceType, _pieceRot)) {
        _pieceR++; _inputTimer = 2;
      }
    }
    if (btn) {
      uint8_t newRot = (_pieceRot + 1) % 4;
      if (_canPlace(_pieceR, _pieceC, _pieceType, newRot)) {
        _pieceRot = newRot; _inputTimer = 8;
      }
    }
  }
...
void TetrisGame::_drawCell(LEDMatrix& matrix, int8_t r, int8_t c, uint8_t colorIdx) {
  matrix.setPixel(r, c + TET_COL_OFF, _color(colorIdx));
}
...
void TetrisGame::draw(LEDMatrix& matrix) {
  ...
  for (int8_t r = 0; r < TET_ROWS; r++)
    for (int8_t c = 0; c < TET_COLS; c++)
      if (_board[r] & (1 << c))
        _drawCell(matrix, r, c, _rowColor[r]);
  ...
  // Level-Anzeige (oben links, 1-5 Punkte)
